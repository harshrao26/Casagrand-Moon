  ...
  return (
    <form className={`space-y-4 ${className}`} onSubmit={handleSubmit}>
      <div className="relative">
        <User
          className="absolute left-4 top-1/2 -translate-y-1/2 text-[#FCB33A]"
          size={20}
        />
        <input
          type="text"
          ...
          className="w-full rounded-2xl border border-zinc-200 bg-zinc-50 py-3.5 pl-12 pr-5 text-sm font-semibold text-zinc-950 outline-none transition placeholder:text-zinc-400 focus:border-[#FCB33A] focus:bg-white focus:ring-4 focus:ring-[#FCB33A]/15"
          required
        />
      </div>
      ...
      <button
        ...
        className="group inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-[#FCB33A] px-6 py-4 text-sm font-bold uppercase tracking-[1.5px] text-black shadow-xl shadow-[#FCB33A]/20 transition hover:bg-zinc-950 hover:text-white disabled:cursor-not-allowed disabled:opacity-70"
      >
        {status === "loading" ? (
          <>
  ...
  return (
    <div className="fixed inset-0 z-[99999] flex items-center justify-center px-4 py-8">
      ...
      <div className="relative max-h-[92vh] w-full max-w-lg overflow-y-auto rounded-[34px] bg-white p-5 shadow-[0_35px_120px_rgba(0,0,0,0.35)] md:p-7">
        <div className="absolute left-1/2 top-0 h-72 w-72 -translate-x-1/2 rounded-full bg-[#FCB33A]/15 blur-[90px]" />

        <div className="relative z-10 mb-6 flex items-start justify-between gap-6">
          <div>
            <p className="mb-3 inline-flex items-center gap-2 rounded-full bg-[#FCB33A]/10 px-3 py-1.5 text-xs font-bold uppercase tracking-[2px] text-[#FCB33A]">
              <CalendarCheck size={14} />
              Casagrand Casablanca
            </p>

            <h2 className="text-3xl font-extrabold leading-tight text-zinc-950 md:text-4xl">
              Book Your <span className="text-[#FCB33A]">Site Visit</span>
            </h2>
